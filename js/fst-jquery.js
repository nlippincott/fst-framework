...
/***********************************************************************
## fst.action - FST Actions Object {#fst-action}
...
which may be specified via [fst.ajax.options](#fst-ajax-options)). However, if a
function exists in the [fst.action](#fst-action) object corresponding to the action name,
that function is invoked instead. For example, if *fst.action.foo* is a
function, the the hyperlink with href '?foo' will trigger this function rather
than invoking an Ajax request. In addition, button elements generated by the
framework with actions beginning with '?' will exhibit the same behavior.
***********************************************************************/
fst.action = new Object();

/***********************************************************************
## fst.ajax (String action, Object options) {#fst-ajax}

Send Ajax request to the framework.

- ***action***: Name of action handler, optional
- ***options***: Key/value pairs to define options
...
/***********************************************************************
## fst.ajax.options (String action, Object options) {#fst-ajax-options}

Register options for Ajax action.

- ***action***: Ajax action (which gets sent to FST controller)
- ***options***: Ajax options

For a given Ajax action, sets the default option to be used when that
action is initiated.
***********************************************************************/
fst.ajax.options = function (action, options) {
	fst.ajax._options[action] = options;
}

/***********************************************************************
## fst.ajax.options_default (Object options) {#fst-ajax-options-default}

Register default Ajax options.

- ***options***: Default Ajax options

Registers the default options for all Ajax actions. These defaults will be
overridden by options defined using [fst.ajax.options](#fst-ajax-options), then will be
subsequently overridden by options provided when calling [fst.ajax](#fst-ajax).
***********************************************************************/
fst.ajax.options_default = function (options)
	{ jQuery.extend(fst.ajax._options._default, options); }

/***********************************************************************
## fst.content (String name, Object options) {#fst-content}

Update framework content section via Ajax.

- ***name***: Name of content area (specified in controller template)
- ***options***: Data to be passed via Ajax, optional
...
Get FST content element by name.

- ***name***: Name of the FST content area (optional)
- **Return**: jQuery object for named content area
...
/***********************************************************************
## fst.content.ready (function fcn) {#fst-content-ready}

Register ready function for framework content update

- ***fcn***: Ready function

Registers a ready function for framework content updates. When framework
content is updated via the [fst.content](#fst-content) function, all registered ready
...
/***********************************************************************
## fst.form.cancel () {#fst-form-cancel}

Default form cancel action
...
specify the cancel option by using fst.form.options. To change
the default behavior, re-define this function by assigning a new function
to [fst.form.cancel](#fst-form-cancel).
***********************************************************************/
fst.form.cancel = function () { fst.redirect.back(); }

/***********************************************************************
## fst.form.data (Object form) {#fst-form-data}

Get form data as an object.

- ***form***: Form object (or selector for form object)
...
fst.form.data = function (form) {
	...
	jQuery(form).find('div[data-fst="form-control-multiple"]').each(function () {
		var name = jQuery(this).attr('data-fst-name');
		data[name] = new Array();
		$(this).find('input[data-fst="form-control-multiple-option"]:checked').each(function () {
...
/***********************************************************************
## fst.form.datepicker (mixed ctl) {#fst-form-datepicker}

Convert FST date control to jQuery datepicker.

- ***ctl***: Form(s), control(s), selector, or undefined
...
/***********************************************************************
## fst.form.error (Object form, String fld, String msg, boolean clear) {#fst-form-error}

Set form field error message.

- ***form***: Form object (or selector for form object)
- ***fld***: Field name
- ***msg***: Error message
- ***clear***: Clear previous error messages (default false)
...
/***********************************************************************
## fst.form.errors (Object form, Object errors) {#fst-form-errors}

Set form error messages.

- ***form*** Form object (or selector for form object)
- ***errors*** Name/value pairs of field names and error messages
...
/***********************************************************************
## fst.form.errors.clear (Object form) {#fst-form-errors-clear}
...
/***********************************************************************
## fst.form.name (mixed form) {#fst-form-name}

Get controller form name.

- ***form***: Form selector or form object
...
fst.form.name = function (form) {
	...
	return jQuery(form).attr('data-fst') == 'form' && /^\?_form=\w+$/.test(action) ? action.substring(7) : false;
}

/***********************************************************************
## fst.form.options (mixed name, Object options) {#fst-form-options}

Set default options for a given form.

- ***name***: Controller form name, form selector, or form object
- ***options***: Name/value pairs of form option
...
fst.form.options = function (name, options) {
	var ctlname = (jQuery.type(name) == 'string' && /^\w+$/.test(name)) ? name : fst.form.name(name);
	fst.form._options[ctlname] = options;
}

/***********************************************************************
## fst.init (Object options) {#fst-init}
...
/***********************************************************************
## fst.replace (String uri) {#fst-replace}

Replaces window location in history.

- ***uri***: Relative or absolute URI

Redirects the browser window while replacing the current page in the
browser history.
The URI behavior is the same as that of [fst.redirect](#fst-redirect).
***********************************************************************/
fst.replace = function (uri) {
	window.location.replace(fst.uri(uri));
}

/***********************************************************************
## fst.replace.home () {#fst-replace-home}

Replaces window location with the application home page.
***********************************************************************/
fst.replace.home = function () { fst.replace(''); }

/***********************************************************************
## fst.form.timeselect (mixed ctl) {#fst-form-timeselect}

Convert FST time control to selection timeselect.
...
jQuery(document).on('click', 'input[type="button"][data-fst-href], button[data-fst-href]', null, function (event) {
	...
	if (href.match(/^\?/)) {
		...
		if (fst.action[action] !== false) {
			fst.action[action] ? fst.action[action].apply(jQuery(this)) : fst.ajax(href.substring(1), { data: jQuery(this).data() });
		}
	}
	else
...
// Set up Ajax forms.
jQuery(document).on('submit', 'form[data-fst="form"][action^="?_form="]', null, function (event) {
	...
	options = jQuery.extend({
			callback: function (response) { },
			callback_fail: function (response) { fst.form.errors(this, response.errors); },
	...
	var file_controls = jQuery(form).find('input[data-fst|="form-control-file"]');
	if (file_controls.length) {
		ajax_options.data = new FormData();
		ajax_options.processData = false;
		ajax_options.contentType = false;
		jQuery.each(options.data, function (key, value) { ajax_options.data.append(key, value); });
			...
			ajax_options.xhr = function () {
				...
				xhr.upload.addEventListener('progress', function (event) {
					if (event.lengthComputable)
						options.progress(Math.round(100 * event.loaded / event.total), event.loaded, event.total);
					else
	...
	jQuery.ajax(ajax_options).done(function (response) {
		response.valid ? options.callback.apply(form, [ response ]) : options.callback_fail.apply(form, [ response ]);
...
//	such forms.
jQuery(document).on('click', 'form[data-fst="form"] button[data-fst="form-cancel"]', null, function (event) {
	var frm = jQuery(this).closest('form');
	var name = fst.form.name(frm);
	var cancel = fst.form._options[name] && fst.form._options[name].cancel ? fst.form._options[name].cancel : fst.form.cancel;
	cancel.apply(frm);
});
