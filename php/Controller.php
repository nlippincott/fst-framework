...
// Revisions, ver 5.1
//	- Generate usage exception in form handler if form not defined
//	- Added null return rule for form validation methods
...
/**
 * @brief FST Controller, abstract base class.
 *
 * A page controller must be derived from this class, and must override
 * function 'init'. A controller may also set member variables 'template'
...
abstract class Controller {
	...
	/**
	 * @brief Registered forms for use with the form processor.
	 *
	...
	/**
	 * @brief Perform controller initialization.
	 *
	...
	/**
	 * @brief Get the controller action (final).
	 * @retval string Action name, or false if none defined
	 *
	...
	/**
	 * @brief Get positional controller argument (final).
	 * @param int $idx Index of the controller argument
	 * @retval string Argument value, or null if no argument
	 *
	...
	/**
	 * @brief Get controller argument string (final).
	 * @retval string Argument value
	 *
	...
	/**
	 * @brief Get controller argument vector (final).
	 * @retval array Array of controller argument strings
	 *
	...
	/**
	 * @brief Get controller name (final).
	 * @retval string Name of the current controller
	 */
	final public function ctrl () { return Framework::ctrlname(); }

	/**
	 * @brief Creates a form and registers it for use with the form handler (final).
	 * @param string $name Form handler name
	 * @param string $id Form ID
	 * @param string $class Form class
	 * @retval object The Form object
	 *
	...
	/**
	 * @brief Get the server's host name (final).
	 * @retval string Host name
	 */
	final protected function host () { return $_SERVER['SERVER_NAME']; }

	/**
	 * @brief Test if controller running as an Ajax call (final).
	 * @retval bool Is running as Ajax flag
	 */
	...
	/**
	 * @brief Test if controller is processing (non-Ajax) POST data (final).
	 * @retval bool Is processing POST data flag
	 */
	final protected function is_post ()
		{ return !$this->is_ajax() && $_SERVER['REQUEST_METHOD'] == 'POST'; }

	/**
	 * @brief Test if running on the SSL port (final).
	 * @retval bool Is running on port 443 (standard SSL port) flag
	 */
	final protected function is_ssl ()
		{ return $_SERVER['SERVER_PORT'] == 443; }

	/**
	 * @brief Exit with a 404 Not Found header.
	 *
	...
	/**
	 * @brief Redirect to another controller or web page.
	...
	/**
	 * @brief Reloads the current controller.
	 *
	...
	/**
	 * @brief Invoke the Ajax handler specified by the action (final).
	 *
	...
	/**
	 * @brief Invoke the content preprocessor (final).
	 *
	...
	/**
	 * @brief Invoke the form handler (final).
	 *
...
//			(!method_exists($this, $method) || $this->$method($form));
	...
	/**
	 * @brief Invoke the page preprocessor (final).
	 *
	...
	/**
	 * @brief Invoke the POST handler (final).
	 *
	...
	/**
	 * @brief Ajax handler for processing registered forms (final).
	 */
	final public function ajax__form ()
		{ return $this->_invoke_form_handler(); }

	/**
	 * @brief Post handler for processing registered forms (final).
	 */
